...
import { DELETE } from '@/app/api/routes-b/invoices/[id]/tags/[tagId]/route'
...
function makeRequest(authHeader = 'Bearer token'): NextRequest {
  return new NextRequest('http://localhost/api/routes-b/invoices/inv-1/tags/tag-1', {
    method: 'DELETE',
...
describe('DELETE /api/routes-b/invoices/[id]/tags/[tagId]', () => {
  beforeEach(() => {
    vi.resetAllMocks()
  })
  ...
  it('returns 401 without a verified bearer token', async () => {
    ...
    const res = await DELETE(makeRequest(''), params as any)
  ...
  it('returns 403 when the invoice belongs to a different user', async () => {
    mockedVerify.mockResolvedValue({ userId: 'privy-1' } as never)
    mockedUserFindUnique.mockResolvedValue(user as never)
    mockedInvoiceFindUnique.mockResolvedValue({ id: 'inv-1', userId: 'user-2' } as never)
    const res = await DELETE(makeRequest(), params as any)
  ...
  it('returns 404 when the tag is not attached to the invoice', async () => {
    mockedVerify.mockResolvedValue({ userId: 'privy-1' } as never)
    mockedUserFindUnique.mockResolvedValue(user as never)
    mockedInvoiceFindUnique.mockResolvedValue({ id: 'inv-1', userId: user.id } as never)
  ...
  it('deletes the join row and returns 204 on the happy path', async () => {
    ...
    mockedUserFindUnique.mockResolvedValue(user as never)
    mockedInvoiceFindUnique.mockResolvedValue({ id: 'inv-1', userId: user.id } as never)
    mockedInvoiceTagFindUnique.mockResolvedValue({ invoiceId: 'inv-1', tagId: 'tag-1' } as never)
