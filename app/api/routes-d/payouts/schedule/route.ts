...
export async function POST(request: NextRequest) {
  try {
    ...
    const body: SchedulePayoutRequest = await request.json().catch(() => ({ items: [], scheduledFor: '' }))
    ...
    const usdcBalance = balances.find(
      (b: any) =>
        b.asset_code === 'USDC' && b.asset_issuer === process.env.NEXT_PUBLIC_USDC_ISSUER,
    )
      ...
      return NextResponse.json(
        {
          ...
          details: { required: totalRequired, available: userBalanceUSDC, estimatedFees },
        },
        { status: 400 },
      )
    }
    ...
    return NextResponse.json({ error: error.message || 'Failed to schedule payout' }, { status: 500 })
  }
}
