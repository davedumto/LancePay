...
function postRequest(body: unknown) {
  return new NextRequest('http://localhost/api/routes-d/account/delete', {
    method: 'POST',
...
describe('POST /api/routes-d/account/delete', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  it('returns 401 when the user is not authenticated', async () => {
    ...
    const { POST } = await import('@/app/api/routes-d/account/delete/route')
