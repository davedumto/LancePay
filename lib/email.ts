...
export async function sendInvoiceCreatedEmail(params: {
  ...
  const primaryColor = params.branding?.primaryColor || '#111111'
  const accentColor = params.branding?.accentColor || '#10b981'
  const footerText = params.branding?.footerText || 'LancePay - Get paid globally, withdraw locally'
  const logoHtml = params.branding?.logoUrl
    ? `<img src="${params.branding.logoUrl}" alt="Brand logo" style="max-width: 140px; max-height: 56px; margin-bottom: 12px;" />`
    : ''
  ...
  return sendEmail({
    ...
    html: `
      <div style="font-family: system-ui, sans-serif; max-width: 500px; margin: 0 auto; padding: 24px;">
        ${logoHtml}
        <h2 style="color:${primaryColor}; margin: 0 0 12px 0;">Invoice ${params.invoiceNumber}</h2>
        ...
        <a href="${params.paymentLink}" style="display: inline-block; background: ${accentColor}; color: white; padding: 12px 24px; border-radius: 8px; text-decoration: none;">Pay Now</a>
